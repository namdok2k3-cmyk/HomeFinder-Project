...
export class MapWidget extends SKElement {
  ...
  public updateFeatures(label:string, state:boolean){
    if (label == "Garage"){
      this._model.showGarage = state
    }
    if (label == "Pool"){
      this._model.showPool = state
    }
    if (label == "Waterfront"){
      this._model.showWaterfront = state
    }
  }
  public updateType(label:string){
    this._model.showType=label;
}
public updateRange(
  costMin:number,costMax:number,bathMin:number,bathMax:number,bedMin:number,bedMax:number
): void {
  this._model.costMin=costMin
  this._model.costMax=costMax
  this._model.bathMin=bathMin
  this._model.bathMax=bathMax
  this._model.bedMin=bedMin
  this._model.bedMax=bedMax
}
}