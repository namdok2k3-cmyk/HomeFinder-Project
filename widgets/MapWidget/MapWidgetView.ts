...
export class MapWidgetView {
  ...
  draw(gc: CanvasRenderingContext2D) {
    ...
    this._model.points.forEach((property) => {
      ...
      if (
        ((this._model.showGarage && property.data.features.includes("garage"))||
        (this._model.showPool && property.data.features.includes("pool"))||
        (this._model.showWaterfront && property.data.features.includes("waterfront"))||
        (!this._model.showGarage && !this._model.showWaterfront && !this._model.showPool))&&
        (this._model.showType=="all"||
          (property.data.property_type==this._model.showType)
        ) && property.data.bedrooms>=this._model.bedMin && property.data.bedrooms<=this._model.bedMax &&
        property.data.bathrooms<=this._model.bathMax && property.data.bathrooms>=this._model.bathMin &&
        property.data.price>=this._model.costMin && property.data.price<=this._model.costMax){
